      ...
      >
        ...
        <div className="flex-1 overflow-y-auto py-4 px-3 space-y-6">
          ...
          <div className="pt-2 border-t border-white/[0.08]">
            {!currentUser ? (
              <div className="p-2.5 rounded-2xl bg-gradient-to-br from-blue-950/40 to-indigo-950/40 border border-blue-500/20 space-y-2">
                {isOpen ? (
                  <>
                    <div className="flex items-center gap-2">
                      <div className="w-7 h-7 rounded-lg bg-blue-600/30 border border-blue-400/30 flex items-center justify-center text-blue-400 shrink-0">
                        <UserIcon className="w-3.5 h-3.5" />
                      </div>
                      <div className="leading-tight overflow-hidden">
                        <p className="text-xs font-bold text-white truncate">Player Account</p>
                        <p className="text-[10px] text-slate-400 truncate">Save progress & scores</p>
                      </div>
                    </div>
                    <div className="flex items-center gap-1.5 pt-0.5">
                      <button
                        onClick={onOpenSignup}
                        className="flex-1 py-1.5 px-2 rounded-lg bg-blue-600 hover:bg-blue-500 text-white text-xs font-bold text-center shadow transition-all cursor-pointer"
                      >
                        Sign Up
                      </button>
                      <button
                        onClick={onOpenLogin}
                        className="py-1.5 px-2 rounded-lg bg-white/[0.06] hover:bg-white/[0.1] text-slate-300 hover:text-white text-xs font-semibold text-center transition-all cursor-pointer"
                      >
                        Log In
                      </button>
                    </div>
                  ...
                  <button
                    ...
                    className="w-full flex items-center justify-center p-2 rounded-xl bg-blue-600 hover:bg-blue-500 text-white shadow transition-all cursor-pointer"
                  >
              ...
              <div className="p-2.5 rounded-2xl bg-white/[0.04] border border-white/[0.08] space-y-1.5">
                {isOpen ? (
                  <>
                    <div className="flex items-center justify-between">
                      <div className="flex items-center gap-2 overflow-hidden">
                        <div className="w-7 h-7 rounded-lg bg-gradient-to-br from-blue-500 to-indigo-600 flex items-center justify-center text-white font-bold text-xs shrink-0">
                          {currentUser.username.charAt(0).toUpperCase()}
                        </div>
                        <div className="leading-tight truncate">
                          <span className="text-xs font-bold text-white truncate block">@{currentUser.username}</span>
                          <span className="text-[10px] text-emerald-400 font-mono">
                            {currentUser.isAdmin ? '🛡️ Admin' : '🎮 Player'}
                          </span>
                        </div>
                      </div>
                    </div>
                    <div className="flex items-center gap-1 pt-1.5 border-t border-white/[0.06]">
                      <button
                        ...
                        className="flex-1 py-1 px-1.5 rounded-lg bg-white/[0.05] hover:bg-white/[0.1] text-slate-300 text-[10px] font-semibold text-center transition-colors cursor-pointer"
                      >
                        + New Account
                      </button>
                      <button
                        onClick={onLogout}
                        title="Log out"
                        className="py-1 px-2 rounded-lg bg-rose-950/40 hover:bg-rose-900/60 text-rose-300 text-[10px] font-semibold text-center border border-rose-500/20 transition-colors cursor-pointer"
                      >
                        Log Out
                      </button>
                    </div>
