...
import { ShieldCheck, ShieldAlert, Lock, Unlock, KeyRound, Sparkles, Check, AlertTriangle, Trash2, Edit3 } from 'lucide-react';
    ...
    <section className="mt-14 mb-8 max-w-5xl mx-auto px-4 sm:px-6">
      <div className={`relative overflow-hidden rounded-3xl border transition-all duration-300 p-6 sm:p-8 backdrop-blur-xl ${
        ...
        <div className="relative z-10">
              ...
              <div>
                <div className="flex items-center gap-2">
                  <h3 className="text-base sm:text-lg font-bold text-white font-['Outfit'] tracking-tight">
                    {isAdmin ? 'Admin Moderation Terminal' : 'Secret Access Terminal'}
                  </h3>
                ...
                <p className="text-xs text-slate-400 mt-0.5">
                  {isAdmin 
                    ? 'Full moderation controls unlocked: edit or delete any inappropriate games from the platform.'
                    : 'Scroll down terminal: Enter secret clearance code to unlock admin moderation controls.'}
                </p>
              </div>
            </div>
          ...
          {successNotice && (
            <div className="mt-4 p-3.5 rounded-2xl bg-emerald-950/60 border border-emerald-500/40 text-emerald-200 text-xs sm:text-sm font-medium flex items-center gap-2.5 animate-in fade-in slide-in-from-top-2">
              <Check className="w-5 h-5 text-emerald-400 shrink-0" />
              <span>Admin controls enabled! You can now edit or delete any game you consider inappropriate.</span>
            </div>
          )}

          {/* Unlocked Admin Controls Overview */}
          {isAdmin ? (
            <div className="mt-5 space-y-4">
              ...
              <div className="p-4 rounded-2xl bg-black/40 border border-emerald-500/20 flex items-start gap-3 text-xs text-slate-300">
                ...
                <div>
                  <span className="font-bold text-emerald-300">Moderation Active: </span>
            ...
            <form onSubmit={handleSubmit} className="mt-5 space-y-3">
              ...
              <p className="text-[11px] text-slate-500 font-mono">
                Security clearance level: System Administrator. Grants immediate deletion of inappropriate community uploads.
              </p>
            </form>
          )}
        </div>
